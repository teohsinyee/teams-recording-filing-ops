...
flowchart LR
    ...
    Approvals --> Runner["run-approved-flow.ps1"]
    Runner --> Copier["copy-approved-recordings.ps1"]
    Runner --> VttBot["download-missing-vtt.ps1"]
    ...
    class Generator,UI,Runner,Copier,VttBot,Playwright,VttSave primary
...
- Target folder suggestion:
  infer from configurable `routingRules` in `config.local.json`.
- Copy naming:
